...
void insertTree(Tree **t, Record r) {
    ...
    if (*t == NULL) {
        *t = (Tree *)malloc(sizeof(Tree));
        ...
        (*t)->weight = 0;
        (*t)->reg.key = r.key;
        (*t)->reg.palavra = (char *)malloc(sizeof(char) * 100);
        (*t)->reg.palavras = (vetor_avl *)malloc(sizeof(vetor_avl) * 20);
        (*t)->reg.cont = 0;
        strcpy((*t)->reg.palavra, r.palavra);
        ...
        if (r.key < (*t)->reg.key) {
            ...
            (*t)->reg.palavras[(*t)->reg.cont].palavra = (char *)malloc(sizeof(char) * 100);
            strcpy((*t)->reg.palavras[(*t)->reg.cont].palavra, r.palavra);
            (*t)->reg.cont++;
        }
...
void preordem(Tree *t) {
    if (!(t == NULL)) {
        printf("%d:%s\t", t->reg.key, t->reg.palavra);
        for (int i = 0; i < t->reg.cont; i++) {
            cout << t->reg.key << " " << t->reg.palavras[i].palavra << endl;
        }
        preordem(t->left);
        preordem(t->right);
    }
